...
IF DB_ID('gym_management_system') IS NOT NULL
BEGIN
...
CREATE TABLE WORKOUT_EQUIPMENT (
    workout_equipment_id INT IDENTITY(1,1) PRIMARY KEY,
...
BEGIN
    ...
    UPDATE s
    SET s.subscription_status = 'Active'
    FROM SUBSCRIPTION s
...
SET IDENTITY_INSERT MEMBERSHIP_PLAN ON;
...
SET IDENTITY_INSERT MEMBERSHIP_PLAN OFF;
GO
...
SET IDENTITY_INSERT WORKOUT_PLAN ON;
...
SET IDENTITY_INSERT WORKOUT_PLAN OFF;
GO

SET IDENTITY_INSERT DIET_PLAN ON;
...
SET IDENTITY_INSERT DIET_PLAN OFF;
GO
...
SET IDENTITY_INSERT WORKOUT_EQUIPMENT ON;
...
SET IDENTITY_INSERT WORKOUT_EQUIPMENT OFF;
GO
...
SELECT
    ...
    ROUND(AVG(CAST(f.rating AS DECIMAL(10,2))), 2) AS average_rating
FROM TRAINER t
...
HAVING AVG(CAST(f.rating AS DECIMAL(10,2))) >= 4;
GO
